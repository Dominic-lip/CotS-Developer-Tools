...
VERTICAL_SLICE_AUTOMATION_TESTS = (
    ...
    SIMULATION_AUTOMATION_TEST, RENEWABLE_AUTOMATION_TEST, KNOWLEDGE_AUTOMATION_TEST,
    ECONOMY_AUTOMATION_TEST, LAW_AUTOMATION_TEST, COMBAT_AUTOMATION_TEST,
    WORLD_AUTHORING_AUTOMATION_TEST, SOCIAL_COMMUNICATION_AUTOMATION_TEST,
)
...
def task_119_source_diagnostics() -> dict[str, Any]:
    """Return only TASK-102's token-free production contract source."""
    sources: dict[str, str] = {}
    for relative_path in TASK_119_SOURCE_FILES:
        path = _safe_relpath(relative_path)
        if not path.is_file():
...
def platform_identity_automation(timeout_seconds: int = 300) -> dict[str, Any]:
    ...
    This fixed operation deliberately accepts no test name, executable, address,
    or other process input. It is the production validation counterpart of the
    bounded TASK-102 manifest and cannot be repurposed as a general launcher.
    """
    if status().get("editor_running"):
        raise Refused("close the production editor before running platform identity automation")
    ...
    result = _run(
        [
            str(EDITOR_CMD), str(PROJECT), mcp_override,
            f"-ExecCmds=Automation RunTests {PLATFORM_IDENTITY_AUTOMATION_TEST};Quit",
            "-unattended", "-nop4", "-nosplash", "-NullRHI", "-NoSound",
        ],
        cwd=PRODUCTION,
        timeout=max(60, min(1200, int(timeout_seconds))),
    ...
    log_path = PRODUCTION / "Saved" / "Logs" / "CotS.log"
    expected_success = (
        f"Test Completed. Result={{Success}} Name={{CharacterSelectionContract}} Path={{{PLATFORM_IDENTITY_AUTOMATION_TEST}}}"
    )
    automation_log_tail = ""
    for _ in range(20):
        try:
    ...
    _write_state(
        last_operation="platform-identity-automation",
        platform_identity_automation_exit_code=result["exit_code"],
        platform_identity_automation_test=PLATFORM_IDENTITY_AUTOMATION_TEST,
    )
    return {
        "success": successful,
        "test": PLATFORM_IDENTITY_AUTOMATION_TEST,
...
def spatial_automation(timeout_seconds: int = 300) -> dict[str, Any]:
    """Run TASK-106's exact spatial-travel authority test."""
    if status().get("editor_running"): raise Refused("close the production editor before running spatial automation")
    override = "-ini:EditorPerProjectUserSettings:[/Script/ModelContextProtocolEngine.ModelContextProtocolSettings]:bAutoStartServer=False"
    result = _run([str(EDITOR_CMD),str(PROJECT),override,f"-ExecCmds=Automation RunTests {SPATIAL_AUTOMATION_TEST};Quit","-unattended","-nop4","-nosplash","-NullRHI","-NoSound"],cwd=PRODUCTION,timeout=max(60,min(1200,int(timeout_seconds))),creationflags=NEW_PROCESS_GROUP)
    log=(PRODUCTION/"Saved"/"Logs"/"CotS.log").read_text(encoding="utf-8",errors="replace")[-20000:]
    expected=f"Test Completed. Result={{Success}} Name={{AuthorityContract}} Path={{{SPATIAL_AUTOMATION_TEST}}}"
    return {"success":result["exit_code"]==0 and expected in log and "**** TEST COMPLETE. EXIT CODE: 0 ****" in log,"test":SPATIAL_AUTOMATION_TEST,"automation_log_verified":expected in log,**result}

def simulation_automation(timeout_seconds: int = 300) -> dict[str, Any]:
    if status().get("editor_running"): raise Refused("close the production editor before running simulation automation")
    result=_run([str(EDITOR_CMD),str(PROJECT),"-ini:EditorPerProjectUserSettings:[/Script/ModelContextProtocolEngine.ModelContextProtocolSettings]:bAutoStartServer=False",f"-ExecCmds=Automation RunTests {SIMULATION_AUTOMATION_TEST};Quit","-unattended","-nop4","-nosplash","-NullRHI","-NoSound"],cwd=PRODUCTION,timeout=max(60,min(1200,int(timeout_seconds))),creationflags=NEW_PROCESS_GROUP)
...
def knowledge_automation(timeout_seconds: int = 300) -> dict[str, Any]:
    result=_run([str(EDITOR_CMD),str(PROJECT),"-ini:EditorPerProjectUserSettings:[/Script/ModelContextProtocolEngine.ModelContextProtocolSettings]:bAutoStartServer=False",f"-ExecCmds=Automation RunTests {KNOWLEDGE_AUTOMATION_TEST};Quit","-unattended","-nop4","-nosplash","-NullRHI","-NoSound"],cwd=PRODUCTION,timeout=max(60,min(1200,int(timeout_seconds))),creationflags=NEW_PROCESS_GROUP);log=(PRODUCTION/"Saved"/"Logs"/"CotS.log").read_text(encoding="utf-8",errors="replace")[-20000:];expected=f"Test Completed. Result={{Success}} Name={{KnowledgeLedger}} Path={{{KNOWLEDGE_AUTOMATION_TEST}}}";return {"success":result["exit_code"]==0 and expected in log and "**** TEST COMPLETE. EXIT CODE: 0 ****" in log,"test":KNOWLEDGE_AUTOMATION_TEST,"automation_log_verified":expected in log,**result}

def economy_automation(timeout_seconds: int = 300) -> dict[str, Any]:
    result=_run([str(EDITOR_CMD),str(PROJECT),"-ini:EditorPerProjectUserSettings:[/Script/ModelContextProtocolEngine.ModelContextProtocolSettings]:bAutoStartServer=False",f"-ExecCmds=Automation RunTests {ECONOMY_AUTOMATION_TEST};Quit","-unattended","-nop4","-nosplash","-NullRHI","-NoSound"],cwd=PRODUCTION,timeout=max(60,min(1200,int(timeout_seconds))),creationflags=NEW_PROCESS_GROUP);log=(PRODUCTION/"Saved"/"Logs"/"CotS.log").read_text(encoding="utf-8",errors="replace")[-20000:];expected=f"Test Completed. Result={{Success}} Name={{GoodsLedger}} Path={{{ECONOMY_AUTOMATION_TEST}}}";return {"success":result["exit_code"]==0 and expected in log and "**** TEST COMPLETE. EXIT CODE: 0 ****" in log,"test":ECONOMY_AUTOMATION_TEST,"automation_log_verified":expected in log,**result}
...
def world_authoring_automation(timeout_seconds: int = 300) -> dict[str, Any]:
    result=_run([str(EDITOR_CMD),str(PROJECT),"-ini:EditorPerProjectUserSettings:[/Script/ModelContextProtocolEngine.ModelContextProtocolSettings]:bAutoStartServer=False",f"-ExecCmds=Automation RunTests {WORLD_AUTHORING_AUTOMATION_TEST};Quit","-unattended","-nop4","-nosplash","-NullRHI","-NoSound"],cwd=PRODUCTION,timeout=max(60,min(1200,int(timeout_seconds))),creationflags=NEW_PROCESS_GROUP);log=(PRODUCTION/"Saved"/"Logs"/"CotS.log").read_text(encoding="utf-8",errors="replace")[-20000:];expected=f"Test Completed. Result={{Success}} Name={{RecipeValidation}} Path={{{WORLD_AUTHORING_AUTOMATION_TEST}}}";return {"success":result["exit_code"]==0 and expected in log and "**** TEST COMPLETE. EXIT CODE: 0 ****" in log,"test":WORLD_AUTHORING_AUTOMATION_TEST,"automation_log_verified":expected in log,**result}
...
def platform_security_automation(timeout_seconds: int = 300) -> dict[str, Any]:
    if status().get("editor_running"):
        raise Refused("close the production editor before running platform security automation")
    result = _run([str(EDITOR_CMD), str(PROJECT), "-ini:EditorPerProjectUserSettings:[/Script/ModelContextProtocolEngine.ModelContextProtocolSettings]:bAutoStartServer=False", f"-ExecCmds=Automation RunTests {PLATFORM_SECURITY_AUTOMATION_TEST};Quit", "-unattended", "-nop4", "-nosplash", "-NullRHI", "-NoSound"], cwd=PRODUCTION, timeout=max(60, min(1200, int(timeout_seconds))), creationflags=NEW_PROCESS_GROUP)
    log = (PRODUCTION / "Saved" / "Logs" / "CotS.log").read_text(encoding="utf-8", errors="replace")[-20000:]
    expected = f"Path={{{PLATFORM_SECURITY_AUTOMATION_TEST}}}"
...
def vertical_slice_automation(timeout_seconds: int = 600) -> dict[str, Any]:
    ...
    result=_run([str(EDITOR_CMD),str(PROJECT),"-ini:EditorPerProjectUserSettings:[/Script/ModelContextProtocolEngine.ModelContextProtocolSettings]:bAutoStartServer=False","-ExecCmds=Automation RunTests CotS.;Quit","-unattended","-nop4","-nosplash","-NullRHI","-NoSound"],cwd=PRODUCTION,timeout=max(120,min(1200,int(timeout_seconds))),creationflags=NEW_PROCESS_GROUP)
    log=(PRODUCTION/"Saved"/"Logs"/"CotS.log").read_text(encoding="utf-8",errors="replace")[-50000:]
    covered=all(f"Path={{{test}}}" in log for test in VERTICAL_SLICE_AUTOMATION_TESTS)
    return {"success":result["exit_code"]==0 and covered and "**** TEST COMPLETE. EXIT CODE: 0 ****" in log,"tests":list(VERTICAL_SLICE_AUTOMATION_TESTS),"automation_log_verified":covered,**result}
